...
class MultipleStartLocalSearch(AbstractStrategy):
    ...
    def _solve_steepest(self, s):
        ...
        for i in range(100):
            solution = [i]
            available = list(range(self.instance.length))
            available.remove(i)
            solution.extend(sample(available, int(self.instance.length / 2) - 1))
            init_solutions.append(deepcopy(solution))
        results = dict()
        for i, solution in tqdm(enumerate(init_solutions)):
